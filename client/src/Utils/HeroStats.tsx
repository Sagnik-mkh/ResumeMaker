import { motion } from "framer-motion";
import { heroStats } from "@/constants/ContentConstants";

const HeroStats = heroStats.map((item, index) => (
	<motion.div
		key={item.value}
		initial={{ opacity: 0, y: 20 }}
		animate={{ opacity: 1, y: 0 }}
		transition={{
			delay: 0.45 + index * 0.12,
			duration: 0.45,
			ease: [0.22, 1, 0.36, 1],
		}}
		whileHover={{ y: -8, scale: 1.02 }}
		className="px-5 py-5 transition duration-300 border group rounded-3xl border-accent/32 bg-background/58 backdrop-blur-2xl hover:-translate-y-1 hover:border-accent/20 hover:bg-dark-accent/72 hover:theme-shadow"
	>
		<div className="mb-4 h-px w-14 bg-[linear-gradient(90deg,var(--accent),transparent)] transition-all duration-300 group-hover:w-20 group-hover:opacity-100" />
		<p className="text-2xl font-semibold tracking-tight transition-colors duration-300 text-secondary sm:text-3xl">
			{item.value}
		</p>
		<p className="mt-2 text-sm leading-6 transition-colors duration-300 theme-text-soft hover:theme-text-soft">
			{item.label}
		</p>
	</motion.div>
));

export default HeroStats;
